    ...
    match opts.command {
        ...
        SubCommand::DeletedTweets(DeletedTweets {
            ...
            if report {
                ...
                println!("## Deleted tweets for {}", screen_name);
                println!(
                    "The list below includes {} deleted tweets by [{}](https://twitter.com/{}).",
                    deleted_count, screen_name, screen_name
                );
                if undeleted_count > 0 {
                    println!("There are also {} tweets that are indicated as not currently deleted by the Twitter API that have been scraped from pages of deleted tweets (as replies, etc.).", undeleted_count);
                    println!("These possibly undeleted tweets are included for context and are indicated by a _(live)_ link.");
                }
                ...
                println!("Please note that all tweets quoted here are sourced from the [Wayback Machine](https://web.archive.org) and were not directly accessed through the Twitter API or any Twitter client.");
